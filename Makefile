...
# explicitly. A local build stays "local" even when `git describe` resembles a
# tag, so self-update refuses to replace it without --force (MADR 0005).
BUILD_KIND?=local
...
linux-arm64: ## Compiles for Linux ARM64
	@mkdir -p $(DIST_DIR)
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -trimpath -tags netgo -ldflags "-extldflags '-static' -s -w -X main.RawVersion=$(VERSION) -X main.RawBuildKind=$(BUILD_KIND)" -o $(DIST_DIR)/$(BINARY_NAME)-linux-arm64 ./cmd/$(BINARY_NAME)
